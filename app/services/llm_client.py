...
def chat_complete_json(prompt: str, system_prompt: str, max_retries: int = 3) -> dict:
    """
    Queries Gemini model and forces valid JSON response.
    """
    current_prompt = prompt
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(f"Attempt {attempt + 1}: Invalid JSON returned by Gemini")
    ...
    raise ValueError("Gemini failed to return valid JSON after retries")
