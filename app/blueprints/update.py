        ...
        if mno:
            ...
            if prod_header:
                current_contract_data = {
                    ...
                    "models": ProdAgmtModels.query.filter_by(
                        header_id=prod_header.id
                    ).all(),
                    "rates": ProdAgmtMdlNormal.query.join(ProdAgmtModels)
                    .filter(ProdAgmtModels.header_id == prod_header.id)
                    .all(),
                    "commitments": ProdAgmtCommitment.query.filter_by(
                        header_id=prod_header.id
                    ).all(),
                }

    total_fields = sum(
        [len(header_dict), len(models) * 3, len(rates) * 4, len(commitments) * 4]
    )
...
def upload_signed_report(doc_id):
    ...
    return redirect(
        url_for("update.final_review", doc_id=doc_id, signed_filename=unique_filename)
    )
...
def publish_to_production():
    ...
    if not mno:
        flash(
            f"Operator '{doc.partner_name}' not found. Please add them in the MNO dashboard.",
            "warning",
        )
        return redirect(url_for("dashboard.index"))
    ...
    if incoming_rp:
        ...
        clashing_headers.extend(
            ProdAgmtHeader.query.filter(ProdAgmtHeader.RP == incoming_rp).all()
        )
        # 2. Trim match (catches old DB records with sneaky trailing spaces)
        clashing_headers.extend(
            ProdAgmtHeader.query.filter(
                db.func.trim(ProdAgmtHeader.RP) == incoming_rp
            ).all()
        )
    ...
    for prod_header in unique_clashing:
        ...
        for p_comm in ProdAgmtCommitment.query.filter_by(
            header_id=prod_header.id
        ).all():
            comm_data = {
                c.name: getattr(p_comm, c.name)
                for c in p_comm.__table__.columns
                if c.name not in ["id", "header_id"]
            }
    ...
    for model_json in models_list:
        ...
        print(
            f"📊 INSERTING MODEL: Seq={m_seq}, Name={m_name}, Type={m_type}", flush=True
        )
        ...
        if m_name or m_type:
            p_model = ProdAgmtModels(
                **{
                    ...
                    "MODEL_TYPE": m_type,
                    "MODEL_NAME": m_name,
                    "AGMT_ID": get_val(model_json, "agmt_id")
                    or get_val(header_json, "agmt_id"),
                }
            )
            ...
            for rate_json in rates_list:
                ...
                if (str(r_seq) == str(m_seq)) or (r_seq is None and str(m_seq) == "1"):
                    ...
                    print(
                        f"  👉 RATE FOUND: Type={r_type} | Extracted Charge={r_charge}",
                        flush=True,
                    )
                    print(
                        f"🔥 SQLALCHEMY THINKS CHARGE_FIELD IS: {ProdAgmtMdlNormal.CHARGE_FIELD.type}",
                        flush=True,
                    )
