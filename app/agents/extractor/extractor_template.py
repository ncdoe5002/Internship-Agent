...
class AgmtHeaderStg(BaseModel):
    ...
    agmt_id: Optional[str] = Field(default=None, description="Unique agreement identification number (e.g., 'IOT-2024-05').")
    sender: Optional[str] = Field(default=None, description="Originating party executing the agreement.")
    rp: Optional[str] = Field(default=None, description="Receiving party or roaming partner entity.")
    tap_direction: Optional[str] = Field(default=None, description="Direction of roaming traffic (e.g., 'Inbound', 'Outbound').")
    rev_no: Optional[int] = Field(default=None)
    start_date: Optional[str] = Field(default=None, description="Effective start date in YYYY-MM-DD format.")
    end_date: Optional[str] = Field(default=None, description="Termination or end date in YYYY-MM-DD format.")
    remarks: Optional[str] = Field(default=None, description="General notes or governing clauses.")
    ...
    currency_code: Optional[str] = Field(default=None, description="ISO currency code (e.g., 'EUR', 'USD').")
    ...
    total_agmt_month: Optional[int] = Field(default=None, description="Total duration in full months.")
...
class AgmtModelsStg(BaseModel):
    ...
    model_seq: Optional[int] = Field(default=None, description="Sequential index of the model.")
    model_type: Optional[str] = Field(default=None, description="Pricing model type (e.g., 'Tiered', 'Flat Rate').")
    model_name: Optional[str] = Field(default=None, description="Descriptive name of the model.")
...
class AgmtMdlNormalStg(BaseModel):
    ...
    rec_type: Optional[str] = Field(default=None, description="Service record type (e.g., 'MOC', 'SMS', 'Data').")
    zone_code: Optional[str] = Field(default=None, description="Target zone or destination group code.")
    rate_currency: Optional[str] = Field(default=None, description="ISO currency code for the rate.")
    pra_rate_type: Optional[str] = Field(default=None, description="Structure type (e.g., 'Linear', 'Incremental', 'Flat').")
    disc_rate_perc: Optional[float] = Field(default=None, description="Percentage discount applied to standard IOT rates.")
    charge_include_tax: Optional[bool] = Field(default=None)
    charge_field: Optional[float] = Field(default=None, description="Actual numerical rate charged per unit (e.g., 0.15, 0.035).")
    model_seq: Optional[int] = Field(default=None, description="Sequence mapping to parent model.")
    agmt_id: Optional[str] = Field(default=None, description="Parent agreement ID.")


class AgmtCommitment(BaseModel):
    """Send-or-pay / traffic allowance commitments per agreement."""
    commitment_name: Optional[str] = Field(default=None, description="Descriptive commitment title.")
    commitment_type: Optional[str] = Field(default=None, description="Structure type (e.g., 'Send or Pay', 'Volume').")
    direction: Optional[str] = Field(default=None, description="'Inbound' or 'Outbound'.")
    amount: Optional[float] = Field(default=None, description="Financial monetary target value.")
    capture_rate_pct: Optional[float] = Field(default=None)
    party_from: Optional[str] = Field(default=None, description="Committed party paying/sending.")
...
class IOTAgreement(BaseModel):
    ...
    normal_model: Optional[List[AgmtMdlNormalStg]] = Field(
        default_factory=list,
        description="Detailed service rate rules."
    )
    commitment: Optional[List[AgmtCommitment]] = Field(
        default_factory=list,
        description="Financial or volume commitment obligations."
    )
